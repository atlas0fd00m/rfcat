...
from builtins import bytes, hex, object, range
...
class EnDeCode(object):
    def encode(self, msg):
        raise Exception(
            "EnDeCode.encode() not implemented.  Each subclass must implement their own"
        )
...
class NICxx11(USBDongle):
    ...
    def __init__(
        self, idx=0, debug=False, copyDongle=None, RfMode=RFST_SRX, safemode=False
    ):
    ...
    def setFreq(self, freq=902000000, mhz=24, radiocfg=None, applyConfig=True):
        ...
        if (
            (freq > FREQ_EDGE_900 and freq < FREQ_MID_900)
            or (freq > FREQ_EDGE_400 and freq < FREQ_MID_400)
            or (freq < FREQ_MID_300)
        ):
            # select low VCO
            radiocfg.fscal2 = 0x0A
        elif freq < 1e9 and (
            (freq > FREQ_MID_900) or (freq > FREQ_MID_400) or (freq > FREQ_MID_300)
        ):
            # select high VCO
        ...
        if applyConfig:
            ...
            self.poke(
                FREQ2,
                struct.pack(
                    "3B", self.radiocfg.freq2, self.radiocfg.freq1, self.radiocfg.freq0
                ),
            )
    ...
    def setMdmModulation(self, mod, radiocfg=None, invert=False):
        ...
        if (mod) & ~MDMCFG2_MOD_FORMAT:
            raise Exception(
                "Please use constants MOD_FORMAT_* to specify modulation and "
            )
    ...
    def setMdmChanSpc(
        self, chanspc=None, chanspc_m=None, chanspc_e=None, mhz=24, radiocfg=None
    ):
        """
        ...
        * chanspc_m and chanspc_e
        ...
        if chanspc != None:
            for e in range(4):
                m = int(
                    (
                        (old_div(chanspc * pow(2, 18), (1000000.0 * mhz * pow(2, e))))
                        - 256
                    )
                    + 0.5
                )  # rounded evenly
        ...
        if chanspc_e is None or chanspc_m is None:
            raise Exception(
                "ChanSpc does not translate into acceptable parameters.  Should you be changing this?"
            )
    ...
    def makePktVLEN(self, maxlen=RF_MAX_TX_BLOCK, radiocfg=None):
        ...
        if maxlen > RF_MAX_TX_BLOCK:
            raise Exception(
                "Packet too large (%d bytes). Maximum variable length packet is %d bytes."
                % (maxlen, RF_MAX_TX_BLOCK)
            )
    ...
    def makePktFLEN(self, flen=RF_MAX_TX_BLOCK, radiocfg=None):
        ...
        if flen > EP5OUT_BUFFER_SIZE - 4:
            raise Exception(
                "Packet too large (%d bytes). Maximum fixed length packet is %d bytes."
                % (flen, EP5OUT_BUFFER_SIZE - 6)
            )
    ...
    def setFsIF(self, freq_if, mhz=24, radiocfg=None):
        ...
        ifBits = int(ifBits + 0.5)  # rounded evenly

        if ifBits > 0x1F:
            raise Exception(
                "FAIL:  freq_if is too high?  freqbits: %x (must be <0x1f)" % ifBits
            )
    ...
    def setMdmChanBW(self, bw, mhz=24, radiocfg=None):
        ...
        for e in range(4):
            m = int(
                ((old_div(mhz * 1000000.0, (bw * pow(2, e) * 8.0))) - 4) + 0.5
            )  # rounded evenly
            if m < 4:
                chanbw_e = e
                chanbw_m = m
        ...
        if chanbw_e is None:
            raise Exception(
                "ChanBW does not translate into acceptable parameters.  Should you be changing this?"
            )
    ...
    def setMdmDRate(self, drate, mhz=24, radiocfg=None):
        ...
        for e in range(16):
            m = int(
                (old_div(drate * pow(2, 28), (pow(2, e) * (mhz * 1000000.0))) - 256)
                + 0.5
            )  # rounded evenly
            if m < 256:
                drate_e = e
        ...
        if drate_e is None:
            raise Exception(
                "DRate does not translate into acceptable parameters.  Should you be changing this?"
            )

        drate = 1000000.0 * mhz * (256 + drate_m) * pow(2, drate_e) / pow(2, 28)
        if self._debug:
            print(
                "drate_e: %x   drate_m: %x   drate: %f Hz" % (drate_e, drate_m, drate)
            )
    ...
    def setMdmDeviatn(self, deviatn, mhz=24, radiocfg=None):
        ...
        for e in range(8):
            m = int(
                (old_div(deviatn * pow(2, 17), (pow(2, e) * (mhz * 1000000.0))) - 8)
                + 0.5
            )  # rounded evenly
            if m < 8:
                dev_e = e
        ...
        if dev_e is None:
            raise Exception(
                "Deviation does not translate into acceptable parameters.  Should you be changing this?"
            )
    ...
    def RFxmit(self, data, repeat=0, offset=0):
        ...
        self.send(
            APP_NIC,
            NIC_XMIT,
            b"%s" % struct.pack("<HHH", len(data), repeat, offset) + data,
            wait=wait,
        )
    ...
    def RFxmitLong(self, data, doencoding=True):
        ...
        retval, ts = self.send(
            APP_NIC,
            NIC_LONG_XMIT,
            b"%s" % struct.pack("<HB", datalen, preload)
            + data[: RF_MAX_TX_CHUNK * preload],
            wait=wait * preload,
        )
        ...
        for chidx in range(preload, chlen):
            ...
            while error == RC_TEMP_ERR_BUFFER_NOT_AVAILABLE:
                retval, ts = self.send(
                    APP_NIC,
                    NIC_LONG_XMIT_MORE,
                    b"%s" % struct.pack("B", len(chunk)) + chunk,
                    wait=wait,
                )
                error = struct.unpack(b"<B", retval[0:1])[0]
            if error:
        ...
        retval, ts = self.send(
            APP_NIC, NIC_LONG_XMIT_MORE, b"%s" % struct.pack("B", 0), wait=wait
        )
        return struct.unpack("<b", retval[0:1])[0]

    def RFtestLong(
        self,
        data=b"BLAHabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZblahaBcDeFgHiJkLmNoPqRsTuVwXyZBLahAbCdEfGhIjKlMnOpQrStUvWxYz",
    ):
        datalen = len(data)
        ...
        retval, ts = self.send(
            APP_NIC,
            NIC_LONG_XMIT,
            b"%s" % struct.pack("<H", datalen) + chunks[0],
            wait=1000,
        )
        sys.stderr.write("=" + repr(retval))
    ...
    def RFlisten(self):
        ...
        while not keystop():
            try:
                ...
                print(
                    "(%5.3f) Received:  %s  | %s"
                    % (t, hexlify(y).decode(), makeFriendlyAscii(y))
                )
    ...
    def RFcapture(self):
        ...
        capture = []
        print(
            "Entering RFlisten mode...  packets arriving will be displayed on the screen (and returned in a list)"
        )
        print("(press Enter to stop)")
        while not keystop():
            try:
        ...
        if IdentSyncWord:
            if lowball <= 1:
                ...
                if SyncWordMatchList != None:
                    print(
                        "  seeking one of: %s"
                        % repr([hex(x) for x in SyncWordMatchList])
                    )
        ...
        while not keystop():
            try:
                ...
                if IdentSyncWord:
                    ...
                    if len(poss):
                        print(
                            "  possible Sync Dwords: %s" % repr([hex(x) for x in poss])
                        )
                        for dw in poss:
                            lst = retval.get(dw, 0)
                            lst += 1
    ...
    def lowball(
        ...
        if hasattr(self, "_last_radiocfg") and len(self._last_radiocfg):
            print(
                "not saving radio state.  already have one saved.  use lowballRestore() to restore the saved config and the next time you run lowball() the radio config will be saved."
            )
        else:
    ...
    def lowballRestore(self):
        if not hasattr(self, "_last_radiocfg"):
            raise Exception(
                "lowballRestore requires that lowball have been executed first (it saves radio config state!)"
            )
    ...
    def reprAESMode(self):
        ...
        if aesmode & AES_CRYPTO_IN_ENABLE:
            output.append(
                "Crypt RF Input:      %s"
                % ("Decrypt", "Encrypt")[(aesmode & AES_CRYPTO_IN_TYPE)]
            )
        else:
        ...
        if aesmode & AES_CRYPTO_OUT_ENABLE:
            output.append(
                "Crypt RF Output:     %s"
                % ("Decrypt", "Encrypt")[(aesmode & AES_CRYPTO_OUT_TYPE) >> 2]
            )
        else:
    ...
    def reprPacketConfig(self, radiocfg=None):
        ...
        output = []
        output.append(
            "Sync Word:           0x%.2X%.2X" % (radiocfg.sync1, radiocfg.sync0)
        )
        ...
        output.append(
            "Data Whitening:      %s"
            % ("off", "ON (but only with cc2400_en==0)")[whitedata]
        )
    ...
    def reprModemConfig(self, mhz=24, radiocfg=None):
        ...
        output.append(
            "DC Filter:           %s"
            % (("enabled", "disabled")[self.getEnableMdmDCFilter(radiocfg)])
        )
    ...
    def checkRepr(self, matchstr, checkval, maxdiff=0):
        ...
        try:
            ...
            if abs(f - checkval) <= maxdiff:
                ...
                print(
                    " *FAILED* reprRadioConfig test: %s %s %s"
                    % (repr(line), repr(val), checkval)
                )
...
def unittest(dongle):
    ...
    for fif, foff in ((164062, 1), (140625, 2), (187500, 3)):
        ...
        if abs(nfif - fif) > 5:
            print(
                " *FAILED* get/setFsIFandOffset():  %d : %f (diff: %f)"
                % (fif, nfif, nfif - fif)
            )
        else:
            print(
                "  passed: get/setFsIFandOffset():  %d : %f (diff: %f)"
                % (fif, nfif, nfif - fif)
            )

        if foff != nfoff:
            print(
                " *FAILED* get/setFsIFandOffset():  %d : %d (diff: %d)"
                % (foff, nfoff, nfoff - foff)
            )
        else:
            print(
                "  passed: get/setFsIFandOffset():  %d : %d (diff: %d)"
                % (foff, nfoff, nfoff - foff)
            )
