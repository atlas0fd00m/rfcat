...
class MAC_Data:
    def __init__(self):
        ...
        self.MAC_threshold = (
            6  # when the T2 clock as overflowed this many times, change channel
        )
        ...
        self.desperatelySeeking = (
            0  # this should be unnecessary, and should instead use mac_state?
        )
        self.txMsgIdx = 0
        self.txMsgIdxDone = 0
...
class fakeDongle:
    ...
    def controlMsg(self, flags, request, buf, value, index, timeout):
        ...
        try:
            ...
            if flags & USB_BM_REQTYPE_DIR_IN:
                logger.info(
                    "<= fakeDoer.controlMsg(flags=0x%x, request=%r, buf=%r, value=%r, index=%x, timeout=%r)",
                    flags,
                    request,
                    buf,
                    value,
    ...
    def bulkWrite(self, chan, buf, timeout=1):
        try:
            ...
            buflen = len(
                buf
            )  # need to return this, because that's what the libusb interface does.
            self._recvbuf += buf
            ...
            if curbuflen < mlen + 2:
                logger.info(
                    "bulkWrite: returning because buffer isn't big enough: len: %x  need: %x",
                    curbuflen,
                    mlen + 2,
                )
                return buflen
            ...
            if app == APP_SYSTEM:
                if cmd == SYS_CMD_PEEK:
                    ...
                    chunk = data[2 : 2 + size]
                    logger.info(
                        "=>> POKE: pkt:%r\t\tdata:%r\t\tsize:%r\t\taddr:%r\t\t%r",
                        repr(pkt),
                        repr(data),
                        hex(size),
                        hex(addr),
                    ...
                    if len(data) > 1:
                        logger.warning(
                            "ummm. what's this extra data in your SYS_CMD_RFMODE command?"
                        )
                    if len(data) == 0:
            ...
            # handle commands for the NIC app
                ...
                if cmd == NIC_GET_AES_MODE:
                    ...
                    if self.macdata.NumChannels <= MAX_CHANNELS:
                        ...
                        self.txdata(
                            app, cmd, struct.pack("<H", self.macdata.NumChannels)
                        )
                    ...
                    if self.macdata.mac_state in (
                        FHSS_STATE_NONHOPPING,
                        FHSS_STATE_DISCOVERY,
                        FHSS_STATE_SYNCHING,
                    ):
                        self.stop_hopping()
                    elif self.macdata.mac_state == FHSS_STATE_SYNCINGMASTER:
                        self.MAC_do_Master_scanny_thingy()
                    elif self.macdata.mac_state in (
                        FHSS_STATE_SYNCHED,
                        FHSS_STATE_SYNC_MASTER,
                    ):
                        self.begin_hopping(0)
